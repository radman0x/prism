...
class MoveProgress {
  constructor(public currentTime: number, public startPos: Position) {}
}
...
export class PixiRendererService {
  ...
  constructor(private ecs: EcsService) {}
  ...
  init(width: number, height: number): void {
    ...
    this.pixiApp.loader
      ...
      .load(() => {
        this.textures = this.pixiApp.loader.resources[
          this.PRISM_SPRITE_SHEET
        ].textures;
        this.smallTextures = this.pixiApp.loader.resources[
          this.SMALLER_SPRITE_SHEET
        ].textures;
        this.ecs.em.each((e: Entity, r: Renderable) => {
        ...
        this.spriteCreateSubscription = this.ecs.em.monitorComponentType(
          ...
          (change: ComponentChange<Renderable>) =>
            this.updateSpriteRegister(change.id, change.c)
        );
  ...
  private updateSpriteRegister(id: number, r?: Renderable) {
    if (!r) {
      ...
      const texture = r.smaller
        ? this.smallTextures[r.image]
        : this.textures[r.image];
    ...
    this.ecs.em.each((e: Entity, anim: MoveAnimation) => {
      ...
      if (!progress || !deepEqual(progress.startPos, anim.start)) {
        ...
        if (progress.currentTime <= anim.durationMs) {
          ...
          const { x, y } = vecToTarget
            .multiply(progressRatio * this.TILE_SIZE)
            .add(anim.start.multiply(this.TILE_SIZE));

          sprite.position.set(x, y);
        } else {
          console.log(
            `current: ${progress.currentTime} vs length: ${anim.durationMs}`
          );
          sprite.position.set(
            anim.end.x * this.TILE_SIZE,
            anim.end.y * this.TILE_SIZE
          );
          toRemove.push(e.id);
  ...
  private renderLoop(): void {
    ...
    renderables.sort(
      (lhs: Entity, rhs: Entity) =>
        lhs.component(Renderable).zOrder - rhs.component(Renderable).zOrder
    );

    const knowledge = this.ecs.em
      .matching(Knowledge)
      .reduce((accum, curr) => curr, null);
    const positionKnowledge = knowledge
      ? knowledge.component(Knowledge).positions
      : null;
    ...
    for (const e of renderables) {
      ...
      if (positionKnowledge && phy) {
        ...
        const convertRgb = (rgb: [number, number, number]) =>
          modRgbString(ROT.Color.toHex(rgb));
        const colorAdder = (
          cs: string,
          rgbVals: [number, number, number]
        ): string => {
          const combined = ROT.Color.add(
            rgbVals,
            ROT.Color.fromString(cs) as [number, number, number]
          );
        ...
        const lightLevel = this.ecs.em
          .matchingIndex(currPos)
          .filter((e: Entity) => e.has(LightLevel))
          .reduce((accum, curr) => curr, null);

        if (posKnown === KnownState.CURRENT) {
          sprite.visible = true;
          const lightRgb = lightLevel
            ? +convertRgb(lightLevel.component(LightLevel).level)
            : +"0x000000";
    ...
    this.setPixiScale(
      this.targetRenderSize.width,
      this.targetRenderSize.height
    );
  ...
  private adjustSprite(
    ...
    if (r.smaller) {
      if (!r.subPos) {
        const subPos = {
          x: spritePos[0] + randomInt(0, 11),
          y: spritePos[1] + randomInt(0, 11)
        };
        const replace = new Renderable(
          r.image,
          r.zOrder,
          r.shrinkFactor,
          r.smaller,
