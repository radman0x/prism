...
export class AIController implements System {
  ...
  constructor(private wallClockId: number) {}
  ...
  update(em: EntityManager) {
    ...
    em.each(
      (aiEntity: Entity, ai: AI, p: Position) => {
        ...
        if (actionPoints >= ai.actionCost) {
          ...
          const closest = this.selectMoveTarget(
            aiPos,
            aiPositions,
            distanceMap,
            em
          ...
          em.setComponent(
            aiEntity.id,
            new AI(ai.lastActionTick + ai.actionCost, ai.actionCost)
          );
        }
      },
      AI,
  ...
  private selectMoveTarget(
    ...
    const playerAdjacent =
      xyPositionsAround(aiPos.asCoord()).filter(
        (c: Coord) => distanceMap.get(Position.fromCoord(c)) === 0
      ).length !== 0;
  ...
  private chooseOptimalPosition(
    ...
    const surrounding = xyPositionsAround(aiCoord).filter(
      (c: Coord) =>
        !this.obstacleAtPos(Position.fromCoord(c), em) &&
        !aiPositions.has(Position.fromCoord(c))
    );
    let closest: { pos: Position | null; distance: number } = {
      pos: null,
      distance: Infinity
    };
    ...
    for (let neighbourCoord of surrounding) {
      ...
      closest =
        neighbourDist < closest.distance
          ? { pos: Position.fromCoord(neighbourCoord), distance: neighbourDist }
          : closest;
      closest =
        neighbourDist === closest.distance && cardinal
          ? { pos: Position.fromCoord(neighbourCoord), distance: neighbourDist }
          : closest;
    }
  ...
  private chooseRandomPosition(
    ...
    const aiCoord = aiPos.asCoord();
    const surrounding = xyPositionsAround(aiCoord).filter(
      (c: Coord) => !this.fillAtPos(Position.fromCoord(c), em)
    );
    if (surrounding.length === 0) {
      return null;
    } else {
      const selectedPos = Position.fromCoord(
        surrounding[randomInt(0, surrounding.length - 1)]
      );
