...
export class Dismantle implements System {
  ...
  update(em: EntityManager): void {
    em.each((dismantlerEntity: Entity, m: PhysicalMove) => {
      ...
      const destructibleAt = em
        .matchingIndex(m.target)
      ...
      if (destructibleAt !== null) {
        ...
        console.log(
          `Destructible suffering ${
            dismantlerEntity.component(Combat).damage
          } damage`
        );
        em.setComponent(
          destructibleAt.id,
          new Health(
            dHealth.current - dismantlerEntity.component(Combat).damage,
            dHealth.max
          )
        );
