...
export class Lighting implements System {
  ...
  update(em: EntityManager): void {
    ...
    em.each(
      (e: Entity, p: Position, y: Physical) => {
    ...
    em.each(
      (lsEntity: Entity, l: LightSource, p: Position) => {
  ...
  private lightBlockAtPos(pos: Position, em: EntityManager): boolean {
    return (
      em
        ...
        .filter(
          (fe: Entity) =>
            fe.component(Position).z === 0 &&
            fe.has(Physical) &&
            fe.component(Physical).size === Size.FILL
        ).length !== 0
    );
  }
}
