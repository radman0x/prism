...
export class Projectiles implements System {
  ...
  constructor(prs: PixiRendererService) {
    this.pixiRenderService = prs;
  }

  update(em: EntityManager) {
    em.each(
      (e: Entity, v: Velocity, p: Position) => {
        const HACK_RANGE = 20;
          ...
          (x: number, y: number) => {
            ...
            if (!strikePoint && !deepEqual(currPos, p)) {
              const physicalAt = em
                ...
                .filter((e: Entity) => e.has(Physical));
              const strikeTarget = physicalAt.find((e: Entity) => {
                return (
                  e.component(Physical).size >= Size.MEDIUM &&
                  e.component(Physical).size !== Size.FILL
                );
              });
              if (strikeTarget) {
                console.log(`possible strike target: ${strikeTarget.id}`);
                const baseChance = 25;
                const targetingBonus =
                  e.has(Aimed) && deepEqual(e.component(Aimed).target, currPos)
                    ? 50
                    : 0;
                ...
                if (randomInt(1, 100) <= strikeChance) {
                  if (strikeTarget.has(Health)) {
                    ...
                    if (targetHealth) {
                      em.setComponent(
                        strikeTarget.id,
                        new Health(targetHealth.current - 15, targetHealth.max)
                      );
                      strikePoint = { pos: currPos, wall: false };
                      console.log(
                        `target struck and now has: ${
                          strikeTarget.component(Health).current
                        } hitpoints`
                      );
                    }
                  }
                ...
                physicalAt.find(
                  (pe: Entity) => pe.component(Physical).size === Size.FILL
                )
              ) {
          ...
          for (const pos of dropCandidates) {
            if (!this.fillAtPos(pos, em)) {
              em.createEntity(
                ...
                pos,
                new MoveAnimation(new Position(p.x, p.y, 0), pos, 70, ``)
              );
