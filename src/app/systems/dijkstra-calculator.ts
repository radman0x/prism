...
import { DijkstraMap, Position, Physical, Size, Player } from '../components.model';
  ...
  update(em: EntityManager) {
    ...
    em.each( (e: Entity, p: Player) => {
      ...
      if ( ! oldMapLocus || ! deepEqual(targetPos, oldMapLocus) ) {
        ...
        let walkableMap = new ValueMap<Position, {blocked: boolean, hasFloor: boolean}>();
        em.each( (e: Entity, p: Position, py: Physical) => {
        ...
        for (const [pos, details] of walkableMap) {
          if ( ! details.blocked && details.hasFloor ) {
            ...
            for (const neighbourCoord of surrounding) {
              ...
              if ( neighbourWalkable && ! neighbourWalkable.blocked && neighbourWalkable.hasFloor ) {
        ...
        for ( const [nodeId, entry] of Object.entries(alg.dijkstra(worldGraph, asId(targetPos))) ) {
