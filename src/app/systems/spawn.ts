      ...
      (e: Entity, s: Spawner, p: Position) => {
        ...
        for (; spawnerAP >= s.spawnRate; spawnerAP -= s.spawnRate) {
          ...
          if (randomInt(0, 100) <= spawnChance) {
            ...
            const newSpawn = em.createEntity(spawnAt, ...s.spawnComponents).id;
            em.setComponent(newSpawn, new AI(wallClock.currentTick, 150));
            console.log(
              `new enemy spawned, total now: ${em.matching(AI).length}`
            );
          }
        }
        em.setComponent(
          ...
          new Spawner(
            s.spawnComponents,
            s.spawnRate,
            wallClock.currentTick - spawnerAP,
            s.spawnChance
          )
        );
      },
  ...
  private spawnBlockedAtPos(pos: Position, em: EntityManager): boolean {
    return (
      em
        ...
        .filter(
          (fe: Entity) =>
            fe.has(Physical) && fe.component(Physical).size >= Size.MEDIUM
        ).length !== 0
    );
  }
}
